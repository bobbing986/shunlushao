...
<script>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<script type="text/javascript" src="/Public/static/uploadify/jquery.uploadify.min.js"></script>
		<script type="text/javascript" src="/Public/static/UploadImages.js"></script><div class="main-title cf">
		<h2>
			...
			<?php if(isset($article)): ?>：<a href="<?php echo U('goods/index','cate_id='.$info['category_id'].'&pid='.$article['id']);?>"><?php echo ($article["title"]); ?></a><?php endif; ?>
			]
		</h2>
	</div>
	<!-- 标签页导航 -->
<div class="tab-wrap">
  ...
  <?php if($data[$field['name']]) { $valArr= explode(',',$data[$field['name']]); } else{ $valArr="";} ?> <div class="upload-img-box">
        <?php if(!empty($valArr)): if(is_array($valArr)): $i = 0; $__LIST__ = $valArr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="upload-pre-item">
                    ...
                    <span class='btn-close btn-close-<?php echo ($field["name"]); ?>' title='删除图片'onclick='del(event);'></span>
...
$obj.parents(".upload-pre-item").remove();
   picsbox = $("#upload_picture_<?php echo ($field["name"]); ?>").siblings('.upload-img-box');
            ...
            $('.icon.<?php echo ($field["name"]); ?>').val(picStr);	
			   ...
			   <?php case "num": ?><input type="text" class="text input-mid" name="<?php echo ($field["name"]); ?>" value="<?php echo ($field["value"]); ?>"><?php break;?>
		...
		<div class="form-item cf">
			...
			<a class="btn btn-return" href="<?php echo U('goods/index?cate_id='.$cate_id);?>">返 回</a>
