...
<body>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<div class="main-title">
		<h2>幻灯片管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>

	<div class="cf">
		<a class="btn" href="<?php echo U('add');?>">新 增</a>
		  <button class="btn ajax-post confirm" url="<?php echo U('del');?>" target-form="ids">删 除</button>
       （位置：1-首页，2-频道页， 3-内容页)  <!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入幻灯片名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/index.php?s=/Admin/Slide/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($order["id"]); ?>"></td>
						<td><?php echo ($order["id"]); ?></td><td><a title="编辑" href="<?php echo U('edit?id='.$order['id'].'&pid='.$pid);?>"><?php echo ($order["title"]); ?></a></td>
<td><?php echo ($order["place"]); ?></td>
<td><?php echo ($order["url"]); ?></td>

						

						 <td><img src="<?php echo (get_cover($order["icon"],'path')); ?>"  width="100" height="50"/>（670*400） <a title="编辑" href="<?php echo (get_cover($order["icon"],'path')); ?>" target='_blank'>查看大图</a></td>
