...
<head>
   ...
   <?php if(isset($cate_rows)): ?><link rel="stylesheet" type="text/css" href="/Public/Admin/css/style.css" media="all"><?php else: ?> <link rel="stylesheet" type="text/css" href="/Public/Admin/css/style-default.css" media="all"><?php endif; ?>  
...
<script>
 ...
 <?php if(isset($cate_rows)): ?></div><?php endif; ?> 
	...
	<div class="main-title">
		...
		<?php if(!empty($rightNav)): if(is_array($rightNav)): $i = 0; $__LIST__ = $rightNav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><a href="<?php echo U('goods/index','cate_id='.$nav['id']);?>"><?php echo ($nav["title"]); ?></a>
			<?php if(count($rightNav) > $i): ?><i class="ca"></i><?php endif; endforeach; endif; else: echo "" ;endif; ?>
		<?php if(isset($article)): ?>：<a href="<?php echo U('goods/index','cate_id='.$cate_id.'&pid='.$article['id']);?>"><?php echo ($article["title"]); ?></a><?php endif; ?>
		<?php else: ?>
		<?php if(empty($position)): ?>全部<?php else: ?><a href="<?php echo U('goods/index');?>">全部</a><?php endif; ?>
			<?php if(is_array(C("DOCUMENT_POSITION"))): foreach(C("DOCUMENT_POSITION") as $key=>$vo): if(($position) != $key): ?><a href="<?php echo U('goods/index',array('position'=>$key));?>"><?php echo ($vo); ?></a><?php else: echo ($vo); endif; ?>&nbsp;<?php endforeach; endif; endif; ?>
		]
		<?php if(($allow) == "0"): ?>（该分类不允许发布内容）<?php endif; ?>
		<?php if(count($model) > 1): ?>[ 模型：<?php if(empty($model_id)): ?><strong>全部</strong><?php else: ?><a href="<?php echo U('goods/index',array('pid'=>$pid,'cate_id'=>$cate_id));?>">全部</a><?php endif; ?>
			<?php if(is_array($model)): $i = 0; $__LIST__ = $model;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($model_id) != $vo): ?><a href="<?php echo U('goods/index',array('pid'=>$pid,'cate_id'=>$cate_id,'model_id'=>$vo));?>"><?php echo (get_document_model($vo,'title')); ?></a><?php else: ?><strong><?php echo (get_document_model($vo,'title')); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
		]<?php endif; ?>
		</h2>
	</div>
	...
	<div class="cf">
		<div class="fl">
			<div class="btn-group">
				<?php if(($allow) > "0"): ?><button class="btn document_add" <?php if(count($model) == 1): ?>url="<?php echo U('goods/add',array('cate_id'=>$cate_id,'pid'=>I('pid',0),'model_id'=>$model[0]));?>"<?php endif; ?>>新 增
						<?php if(count($model) > 1): ?><i class="btn-arrowdown"></i><?php endif; ?>
					</button>
					<?php if(count($model) > 1): ?><ul class="dropdown nav-list">
						<?php if(is_array($model)): $i = 0; $__LIST__ = $model;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('goods/add',array('cate_id'=>$cate_id,'model_id'=>$vo,'pid'=>I('pid',0)));?>"><?php echo (get_document_model($vo,'title')); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
					</ul><?php endif; ?>
				<?php else: ?>
            ...
            <button class="btn ajax-post" target-form="ids" url="<?php echo U("goods/setStatus",array("status"=>1));?>">启 用</button>
			<button class="btn ajax-post" target-form="ids" url="<?php echo U("goods/setStatus",array("status"=>0));?>">禁 用</button>
			<button class="btn ajax-post" target-form="ids" url="<?php echo U("goods/move");?>">移 动</button>
			<button class="btn ajax-post" target-form="ids" url="<?php echo U("goods/copy");?>">复 制</button>
			...
			<button class="btn ajax-post confirm" target-form="ids" url="<?php echo U("goods/setStatus",array("status"=>-1));?>">删 除</button>
